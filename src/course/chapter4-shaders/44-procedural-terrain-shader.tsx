...
import terrainVertexShader from './shaders/44/terrain/vertext.glsl';
import terrainFragmentShader from './shaders/44/terrain/fragment.glsl';
...
const Page: FC = () => {
  ...
  useEffect(() => {
     ...
     */
    // Geometry
    const geometry = new THREE.PlaneGeometry(10, 10, 500, 500); // super high subdivision
    geometry.deleteAttribute('uv');
    geometry.deleteAttribute('normal'); // these 2 attributes not being used, delete for better performance
    ...
    const uniforms = {
      ...
      uWarpFreqncy: new THREE.Uniform(5.0),
    ...
    gui.add(uniforms.uWarpFreqncy, 'value', 0, 10, 0.001).name('uWarpFreqncy');
    ...
    terrain.customDepthMaterial = depthMaterial; // fix shadow
    ...
    // boardHole.updateMatrixWorld(); // can see the bottom of the hole

    //Evaluate
    const evaluator = new Evaluator();
    const board = evaluator.evaluate(boardFill, boardHole, SUBTRACTION); // subtract the hole from the fill
    board.geometry.clearGroups(); // clear all groups vertex
    board.material = new THREE.MeshStandardMaterial({
      color: '#fff',
      metalness: 0,
      roughness: 0.3,
    }); // apply the easy simple material
    board.castShadow = true;
    board.receiveShadow = true;
    scene.add(board); // board is Brush instance, also a THREE.Object3D
